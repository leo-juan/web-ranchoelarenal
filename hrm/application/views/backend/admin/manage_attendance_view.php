                ...
                <table class="table table-bordered" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th style="width:30%"><?php echo get_phrase('status'); ?></th>
                            <th style="width: 40%;"><?php echo get_phrase('reason_of_absence'); ?></th>
                            <th style="width:10%"><?php echo get_phrase('Hora entrada'); ?></th>
                            <th style="width:10%"><?php echo get_phrase('Hora salida'); ?></th>
                        </tr>
...
<!-- codigo juan //////////////////////////////////////////////////////////////-->
                                     <td>
                                        <span style="display: block;" id="entrar_<?php echo $row['attendance_id']; ?>">
                                            <input type="time" name="hora_entrada_<?php echo $row['attendance_id']; ?>"
                                                class="form-control" value="<?php echo $row['hora_entrada']; ?>" />
                                        </span>
                                        <?php if($row['status'] == 2) { ?>
                                            <span style="display: none;" id="entrar2_<?php echo $row['attendance_id']; ?>">
                                                <input type="text" name="hora_entrada_<?php echo $row['attendance_id']; ?>"
                                                    class="form-control" value="<?php echo $row['hora_entrada']; ?>" />
                                            </span>
                                        <?php } if($row['status'] == 1) {?>
                                            <span style="display: block;" id="entrar2_<?php echo $row['attendance_id']; ?>"></span>
                                        <?php } ?>
                                    </td>

                                    <td>
                                        <span style="display: block;" id="salir_<?php echo $row['attendance_id']; ?>">
                                            <input type="time" name="hora_salida_<?php echo $row['attendance_id']; ?>"
                                                class="form-control" value="<?php echo $row['hora_salida']; ?>" />
                                        </span>
                                        <?php if($row['status'] == 2) { ?>
                                            <span style="display: none;" id="salir2_<?php echo $row['attendance_id']; ?>">
                                                <input type="text" name="hora_salida_<?php echo $row['attendance_id']; ?>"
                                                    class="form-control" value="<?php echo $row['hora_salida']; ?>" />
                                            </span>
                                        <?php } if($row['status'] == 1) {?>
                                            <span style="display: block;" id="salir2_<?php echo $row['attendance_id']; ?>"></span>
                                        <?php } ?>
                                    </td> 
                                </tr>
...
<script type="text/javascript">
    ...
    function get_reason_holder(status, attendance_id)
    {
        ...
        if(status == 1)
            ...
            $('#entrar2_' + attendance_id).attr('style', 'display: none;');
        
        if(status == 1)
           
            $('#entrar_' + attendance_id).attr('style', 'display: block;');
        if(status == 2)
           
            $('#entrar_' + attendance_id).attr('style', 'display: none;');

            $('#salir2_' + attendance_id).attr('style', 'display: none;');
        
        if(status == 1)
           
            $('#salir_' + attendance_id).attr('style', 'display: block;');
        if(status == 2)
           
            $('#salir_' + attendance_id).attr('style', 'display: none;');
    }
